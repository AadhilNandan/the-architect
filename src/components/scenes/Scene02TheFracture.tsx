...
interface Scene02TheFractureProps {
  isUnsealed?: boolean;
}

export const Scene02TheFracture: React.FC<Scene02TheFractureProps> = ({ isUnsealed = false }) => {
  ...
  // Compute 3-Keyframe Timeline (Civilization -> Breaking -> Ruin)
  // Keyframe 1: fracture_01_civilization (Establish hold 0.00 -> 0.22, fade out 0.22 -> 0.38)
  const plate1Opacity = Math.max(0, Math.min(1.0, 1 - Math.max(0, (scrollProgress - 0.22) / 0.16)));
  
  // Keyframe 2: fracture_02_breaking (Enter 0.22 -> 0.38, Register Hold 0.38 -> 0.48, fade out 0.48 -> 0.64)
  ...
  // Keyframe 3: fracture_03_ruin (Enter 0.48 -> 0.64, Contemplation Hold 0.64 -> 0.82, yield to transition 0.82 -> 0.94)
  ...
  // Keyframe 4 / Transition Plate: transition_02_03 (Enter 0.80 -> 0.92, Hold 0.92 -> 1.00)
  const transitionOpacity = Math.max(0, Math.min(1.0, (scrollProgress - 0.80) / 0.12));

  // Continuous Camera Movement (Scale, Push, Parallax)
  const cameraScale = 1.06 - scrollProgress * 0.05;
  const cameraTranslateY = scrollProgress * -32;
  const cameraTranslateX = Math.sin(scrollProgress * Math.PI) * -10;
  ...
  // Canonical Narration (Reveal 0.54 -> 0.66, Stable Reading Hold 0.66 -> 0.84, Soft Exit 0.84 -> 0.92)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Keyframe 1: fracture_01_civilization (The Golden Empire) */}
        <div
          style={{
            position: 'absolute',
            inset: '-5% -4% -5% -4%',
        ...
        {/* Keyframe 2: fracture_02_breaking (Reality Cleaving) */}
        <div
          style={{
            position: 'absolute',
            inset: '-5% -4% -5% -4%',
            opacity: plate2Opacity,
            transform: `translate3d(${cameraTranslateX * 0.8}px, ${cameraTranslateY * 0.85}px, 0) scale(${cameraScale * 1.02})`,
            transition: 'opacity 0.15s ease-out',
            zIndex: 3,
            pointerEvents: 'none',
            backgroundColor: '#050505',
        ...
        >
          <Image
            src="/images/fracture_02_breaking.jpeg"
            alt="The monumental architecture breaking under reality fracture"
            fill
            sizes="100vw"
            style={{
              objectFit: 'cover',
        ...
        {/* Keyframe 3: fracture_03_ruin (The Shattered Ground) */}
        <div
          style={{
            position: 'absolute',
            inset: '-5% -4% -5% -4%',
            opacity: plate3Opacity,
            transform: `translate3d(${cameraTranslateX * 0.5}px, ${cameraTranslateY * 0.7}px, 0) scale(${cameraScale * 1.04})`,
            transition: 'opacity 0.15s ease-out',
            zIndex: 4,
            pointerEvents: 'none',
            backgroundColor: '#050505',
        ...
        >
          <Image
            src="/images/fracture_03_ruin.jpeg"
            alt="The shattered Roman forum and fallen monolithic columns"
            fill
            sizes="100vw"
            style={{
              objectFit: 'cover',
              objectPosition: 'bottom center',
              filter: 'brightness(0.96) contrast(1.10)',
            }}
          />
        </div>

        {/* Keyframe 4 / Transition Plate: transition_02_03 (The Threshold into the Awakening) */}
        <div
          style={{
            position: 'absolute',
            inset: '-5% -4% -5% -4%',
        ...
        {/* Layer 7: Canonical Inscribed Narration & Act Marker (Refined Scale) */}
        <div
        ...
        >
          ...
          {/* Canonical Statement — Reduced scale with environmental breathing room */}
          <h2
